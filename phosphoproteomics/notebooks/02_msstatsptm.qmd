...
Site-level localization filtering is applied to the phospho report at `LOC_PROB_CUTOFF` (default 0.75, Class I sites) before the MSstatsPTM converter — the bulk protein report is left untouched.
...
phospho_path <- file.path(phospho_input_dir, "PCF000612_phospho_GA_MSStats_report.csv")
...
if ("EG.PTMAssayProbability" %in% phospho_header) {
  ...
  loc_prob_col <- NA_character_
  msstats_cols <- setdiff(msstats_cols, "EG.PTMAssayProbability")
}
cat("Localization probability column:",
    ifelse(is.na(loc_prob_col), "NONE — site-level filter will be skipped", loc_prob_col),
    "\n")
...
# 0.75 is the Class I threshold from Olsen et al. 2006 and is the standard
# for library-based DIA searches. If the search mode is later confirmed to be
# library-free (directDIA), Bekker-Jensen et al. 2020 recommend 0.99 instead.
LOC_PROB_CUTOFF <- 0.75

n_before <- nrow(phospho_raw)
cat("Phospho rows before localization filter:", n_before, "\n")

if (is.na(loc_prob_col)) {
  warning("Localization probability column not found in phospho report — site-level filter skipped.")
  cat("WARNING: no localization probability column available; filter skipped.\n")
} else {
  phospho_raw <- phospho_raw[get(loc_prob_col) >= LOC_PROB_CUTOFF]
  n_after <- nrow(phospho_raw)
  pct <- round(100 * n_after / n_before, 2)
  cat("Phospho rows after filter (", loc_prob_col, " >= ", LOC_PROB_CUTOFF, "): ",
      n_after, " (", pct, "% retained)\n", sep = "")
}
